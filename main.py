...
def fetch_projects():
    ...
    params = {
        "morph": "true",
        "searchString": "концепция OR изыскания OR ТЭО OR предпроект OR разработка проектной документации",
    ...
    for attempt in range(3):
        try:
            ...
            if resp.status_code == 429:
                print(f"⏳ Лимит запросов. Ждём 10 сек... (попытка {attempt+1})")
                time.sleep(10)
                continue
...
def main():
    ...
    msg = "📡 <b>Радар: новые объекты РФ (концепция/изыскания)</b>\n\n"
    for p in new:
        msg += f"🏗️ <b>{p['name']}</b>\n"
        msg += f"📅 Дата публикации: {p['date']}\n"
        msg += f"💰 Бюджет: {p['budget']}\n"
        msg += f"👥 Заказчик: {p['customer']}\n"
        msg += f"💼 Инвестор: {p['investor']}\n"
        msg += f"📐 Проектировщик: {p['designer']}\n"
        msg += f"🔨 Генподрядчик: {p['contractor']}\n"
        msg += "────────────────────\n"
